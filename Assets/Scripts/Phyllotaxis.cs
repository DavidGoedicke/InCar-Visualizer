...
public class Phyllotaxis : MonoBehaviour {
  ...
  void Update () {
    if (_useScaleAnimation) {
      if (_useScaleCurve) {
        _scaleTimer += _scaleAnimSpeed * _audioPeer.audioBand[_scaleBand] * Time.deltaTime;
        if (_scaleTimer >= 1) {
          _scaleTimer -= 1;
        }
        _currentScale = Mathf.Lerp(_scaleAnimMinMax.x, _scaleAnimMinMax.y, _scaleAnimCurve.Evaluate(_scaleTimer));
      } else {
        _currentScale = Mathf.Lerp(_scaleAnimMinMax.x, _scaleAnimMinMax.y, _audioPeer.audioBand[_scaleBand]);
      }
    }
    if (_useLerping) {
      if (_isLerping) {
        _lerpPosSpeed = Mathf.Lerp(_lerpPosSpeedMinMax.x, _lerpPosSpeedMinMax.y, _lerpPosAnimCurve.Evaluate(_audioPeer.audioBand[_lerpPosBand]));
        ...
        if (_lerpPosTimer >= 1) {
          ...
          if (_currentIteration > 0 && _currentIteration < _maxIteration) {
            SetLerpPositions();
          } else { //current iteratioin has hit 0 or max iteration
            if (_repeat == true) {
              if (_invert) {
                _forward = !_forward;
                SetLerpPositions();
